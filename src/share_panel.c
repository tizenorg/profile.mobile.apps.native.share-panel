...
EAPI int share_panel_reply(share_panel_h share_panel, bool result)
{
	...
	retvm_if(!share_panel->control,
			SHARE_PANEL_ERROR_NOT_INITIALIZED,
			"No request context. Unable to send app_control result");

	int err = app_control_create(&reply);
	retvm_if(err != APP_CONTROL_ERROR_NONE,
				SHARE_PANEL_ERROR_INVALID_PARAMETER,
				"app_control_create failed: %s", get_error_message(err));
...
EAPI int share_panel_destroy(share_panel_h share_panel)
{
	...
	if (share_panel->control) app_control_destroy(share_panel->control);
	free(share_panel);
